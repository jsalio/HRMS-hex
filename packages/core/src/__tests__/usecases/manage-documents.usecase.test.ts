import { describe, it, expect, mock } from 'bun:test'
import { ManageDocumentsUseCase } from '../../usecases/manage-documents.usecase'
import { NotFoundError, ValidationError } from '../../domain/errors'
import type { IDocumentRepository, EmployeeDocumentData } from '../../contracts/documents'
import type { IEmployeeRepository } from '../../contracts/employees'

const now = new Date('2026-01-01T00:00:00Z')

const mockEmployee = {
  id: 'emp-1', fullName: 'Ana García', documentId: 'DNI-001',
  corporateEmail: 'ana@hrms.com', department: { id: 'd-1', name: 'Engineering' },
  jobTitle: 'Developer', salary: 5000, status: 'ACTIVE' as const,
  hireDate: now, terminationDate: null, createdAt: now, updatedAt: now,
  onboarding: [],
}

const mockDoc: EmployeeDocumentData = {
  id: 'doc-1', employeeId: 'emp-1', templateId: null,
  name: 'Employment Contract', type: 'contract', status: 'PENDING',
  fileUrl: 'https://s3.example.com/doc.pdf', fileHash: null,
  signedAt: null, signedBy: null, archivedAt: null, expiresAt: null,
  renewalNotifiedAt: null, renewedFromId: null,
  createdAt: now, updatedAt: now,
}

function makeRepos() {
  const documentRepo: IDocumentRepository = {
    findByEmployee:  mock(async () => [mockDoc]),
    findById:        mock(async () => mockDoc),
    create:          mock(async () => mockDoc),
    sign:            mock(async () => ({ ...mockDoc, status: 'SIGNED' as const, fileHash: 'a'.repeat(64), signedAt: now, signedBy: 'user-1' })),
    archive:         mock(async () => ({ ...mockDoc, status: 'ARCHIVED' as const, archivedAt: now })),
    findExpiring:    mock(async () => []),
  }

  const employeeRepo = {
    findAll:              mock(async () => ({ data: [], total: 0, page: 1 })),
    findById:             mock(async () => mockEmployee),
    findByEmail:          mock(async () => null),
    findByDocumentId:     mock(async () => null),
    create:               mock(async () => mockEmployee),
    update:               mock(async () => mockEmployee),
    terminate:            mock(async () => mockEmployee),
    findOnboarding:       mock(async () => []),
    updateOnboardingStep: mock(async () => ({} as any)),
  } as unknown as IEmployeeRepository

  return { documentRepo, employeeRepo }
}

describe('ManageDocumentsUseCase', () => {
  describe('createDocument', () => {
    it('createDocument_creates_PENDING_document_for_active_employee', async () => {
      const { documentRepo, employeeRepo } = makeRepos()
      const uc = new ManageDocumentsUseCase(documentRepo, employeeRepo)

      const result = await uc.createDocument({
        employeeId: 'emp-1', name: 'Contract', type: 'contract',
        fileUrl: 'https://s3.example.com/doc.pdf',
      })

      expect(documentRepo.create).toHaveBeenCalledTimes(1)
      expect(result.id).toBe('doc-1')
    })

    it('createDocument_throws_NotFoundError_when_employee_not_found', async () => {
      const { documentRepo, employeeRepo } = makeRepos()
      ;(employeeRepo.findById as ReturnType<typeof mock>).mockResolvedValue(null)
      const uc = new ManageDocumentsUseCase(documentRepo, employeeRepo)

      await expect(
        uc.createDocument({ employeeId: 'emp-x', name: 'X', type: 'contract', fileUrl: 'https://s3.example.com/x.pdf' })
      ).rejects.toBeInstanceOf(NotFoundError)
    })

    it('createDocument_throws_ValidationError_when_employee_is_INACTIVE', async () => {
      const { documentRepo, employeeRepo } = makeRepos()
      ;(employeeRepo.findById as ReturnType<typeof mock>).mockResolvedValue({ ...mockEmployee, status: 'INACTIVE' })
      const uc = new ManageDocumentsUseCase(documentRepo, employeeRepo)

      await expect(
        uc.createDocument({ employeeId: 'emp-1', name: 'X', type: 'contract', fileUrl: 'https://s3.example.com/x.pdf' })
      ).rejects.toBeInstanceOf(ValidationError)
    })
  })

  describe('signDocument', () => {
    it('signDocument_transitions_to_SIGNED_and_stores_hash', async () => {
      const { documentRepo, employeeRepo } = makeRepos()
      const uc = new ManageDocumentsUseCase(documentRepo, employeeRepo)

      const result = await uc.signDocument('doc-1', 'a'.repeat(64), 'user-1')

      expect(documentRepo.sign).toHaveBeenCalledWith('doc-1', expect.objectContaining({
        fileHash: 'a'.repeat(64),
        signedBy: 'user-1',
      }))
      expect(result.status).toBe('SIGNED')
    })

    it('signDocument_throws_ValidationError_when_already_SIGNED', async () => {
      const { documentRepo, employeeRepo } = makeRepos()
      ;(documentRepo.findById as ReturnType<typeof mock>).mockResolvedValue({ ...mockDoc, status: 'SIGNED' })
      const uc = new ManageDocumentsUseCase(documentRepo, employeeRepo)

      await expect(uc.signDocument('doc-1', 'a'.repeat(64), 'user-1')).rejects.toBeInstanceOf(ValidationError)
      expect(documentRepo.sign).not.toHaveBeenCalled()
    })
  })

  describe('archiveDocument', () => {
    it('archiveDocument_transitions_to_ARCHIVED', async () => {
      const { documentRepo, employeeRepo } = makeRepos()
      ;(documentRepo.findById as ReturnType<typeof mock>).mockResolvedValue({ ...mockDoc, status: 'SIGNED' })
      const uc = new ManageDocumentsUseCase(documentRepo, employeeRepo)

      const result = await uc.archiveDocument('doc-1')

      expect(documentRepo.archive).toHaveBeenCalledWith('doc-1', expect.any(Date))
      expect(result.status).toBe('ARCHIVED')
    })

    it('archiveDocument_throws_ValidationError_when_PENDING', async () => {
      const { documentRepo, employeeRepo } = makeRepos()
      const uc = new ManageDocumentsUseCase(documentRepo, employeeRepo)

      await expect(uc.archiveDocument('doc-1')).rejects.toBeInstanceOf(ValidationError)
      expect(documentRepo.archive).not.toHaveBeenCalled()
    })
  })

  describe('renewDocument', () => {
    it('renewDocument_creates_new_PENDING_with_renewed_from_id', async () => {
      const { documentRepo, employeeRepo } = makeRepos()
      const uc = new ManageDocumentsUseCase(documentRepo, employeeRepo)

      await uc.renewDocument('doc-1', { fileUrl: 'https://s3.example.com/new.pdf' })

      expect(documentRepo.create).toHaveBeenCalledWith(expect.objectContaining({
        renewedFromId: 'doc-1',
        fileUrl: 'https://s3.example.com/new.pdf',
      }))
    })
  })

  describe('listExpiringDocuments', () => {
    it('listExpiringDocuments_delegates_to_repository', async () => {
      const { documentRepo, employeeRepo } = makeRepos()
      ;(documentRepo.findExpiring as ReturnType<typeof mock>).mockResolvedValue([
        { ...mockDoc, employee: { id: 'emp-1', fullName: 'Ana García', corporateEmail: 'ana@hrms.com' } },
      ])
      const uc = new ManageDocumentsUseCase(documentRepo, employeeRepo)

      const result = await uc.listExpiringDocuments(30)

      expect(documentRepo.findExpiring).toHaveBeenCalledWith(30)
      expect(result[0]).toHaveProperty('employee')
    })
  })
})
